...
bool idea_repo_add(const char *submitter, const char *body, int room_vnum) {
    ...
    bool ok;
    if (room_vnum > 0)
        ok = db_query(db, "insert into idea (submitter, body, room_vnum) values ('%s', '%s', %i)",
                      submitter ? submitter : "", body ? body : "", room_vnum);
    else
        ok = db_query(db, "insert into idea (submitter, body) values ('%s', '%s')",
...
bool idea_repo_list(char *out, size_t size, int limit) {
    ...
    if (db_query(db, "select id, date(created_at) as day, submitter, body, room_vnum "
        ...
        while (db_fetch_row(db)) {
            /* "#12 [2026-07-05] Testguy (room 1234): a purge command would
             * help" -- id/date in a dim cyan tag, then the submitter, the
             * room they filed from (if known), and the idea text. */
