...
('settrap', 'Usage: settrap <direction|container|arrow>\n\nA Thief rigs a trap on a closed door in that direction ("set trap\n(door)"), a closed container ("set trap (container)"), or a carried\narrow ("set trap (arrow)"). A door trap springs on whoever walks\nthrough it next; a container trap springs on whoever opens it; an\narrow trap springs on whoever it hits when shot. Each is a one-shot\nhit to a random limb, and a Thief who knows "detect trap" safely\nsteps around a rigged door or container instead (an arrow already in\nflight cannot be detected).\n\nRelated: disarmtrap', 'seed'),
('disarmtrap', 'Usage: disarmtrap <direction|container|arrow>\n\nA Thief who knows "disarm trap" safely removes a trap rigged on a\ndoor, a container, or a carried arrow, with no risk of springing it.\n\nRelated: settrap', 'seed'),
...
UPDATE `help_topic` SET `body` = 'Usage: settrap <direction|container|arrow>\n\nA Thief attempts to rig a trap on a closed door in that direction\n("set trap (door)"), a closed container ("set trap (container)"),\nor a carried arrow ("set trap (arrow)") -- each needs its own skill.\nSuccess depends on your proficiency with that skill (see `skills`) --\na fumbled attempt wastes the try but climbs your proficiency toward\nsuccess next time regardless. Once rigged, a door/container trap\nsprings on whoever walks through/opens it next, and an arrow trap\nsprings on whoever it hits when shot -- a one-shot hit to a random\nlimb each time. A Thief who knows "detect trap" safely steps around\na rigged door or container instead, leaving it rigged for the next\nperson (an arrow already in flight cannot be detected).\n\nRelated: disarmtrap'
  WHERE `name` = 'settrap' AND `updated_by` = 'seed';

UPDATE `help_topic` SET `body` = 'Usage: disarmtrap <direction|container|arrow>\n\nA Thief who knows "disarm trap" attempts to safely remove a trap\nrigged on a door, a container, or a carried arrow, with no risk of\nspringing it either way. Success depends on your proficiency with the\nskill (see `skills`) -- a fumbled attempt leaves the trap still\nrigged, but climbs your proficiency toward success next time\nregardless.\n\nRelated: settrap'
