...
/* `display <item> <person>` (Sneezy port, user 2026-07-12; renamed from
 * `show` to `display`, user 2026-08-22, to free `show` for the vnum
 * lookup tool -- see cmd_show.c). Sneezy's own `show.cc` is a sprawling
 * immortal admin utility (room/zone listings etc) that's really a
 * different feature from what most players mean by "show" -- and those
 * admin views are already covered by this backlog's own `stat`/`zone
 * list` items. This is the ordinary social meaning instead: hold an item
 * up for someone else in the room to see. Purely a message -- the item
 * never changes hands (that's `give`, not yet ported). */
bool cmd_display(descriptor_t *d, const char *args) {
    ...
    if (sscanf(args, "%63s %63s", item_tok, who_tok) != 2) {
        descriptor_send(d, "Usage: display <item> <person>\r\n");
        return true;
    }
