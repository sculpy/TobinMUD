...
announce("smoke_test_trade_attrs", host, port)
...
check("Str: 150" in out and "Points remaining: 0" in out,
...
check("Str: 150" in out, "Strength unchanged after the rejected attempt")
...
check("Dex: 120" in out, "Dexterity unchanged after the rejected attempt")

# Lowering a stat frees room again -- the trade mechanic.
out = step(s, "wis -30 (floor, should succeed and free up room)", "wis -30")
check("Wis: 90" in out and "Points remaining: 30" in out,
...
check("Adjustment must be between -30 and +30" in out, "adjustment beyond -30 is rejected")
check("Wis: 90" in out, "Wisdom unchanged after the rejected attempt")
...
check("Dex: 150" in out and "Points remaining: 0" in out,
...
check("Con: 120" in out, "the rejected attempt left Constitution unchanged")

out = step(s, "reset clears everything", "reset")
check("Points remaining: 30" in out and "Str: 120" in out,
      "reset returns all attributes to base and restores the full pool")

s.close()
announce_done("smoke_test_trade_attrs", host, port)
