...
/* Dragon flight's in-progress task (`fly` command, cmd_fly.c) -- see
 ...
 * validated by cmd_fly.c: arms the countdown, sets the wait-lockout for
 * the whole trip (same convention as spellcast_start()'s being_set_wait()
 * call), and moves `ch` into the first shared sky waypoint immediately
 * with its own departure flavor. `dest_vnum` is the real roost
 * fly_tick_run() lands them in on the final tick. */
 ...
 * on the final tick, ~3s apart), aborting nothing mid-flight -- once
 * paid for and airborne over open sky there's nothing to interrupt it
 * with (see cmd_fly.c's doc comment). Pulse-registered in main.c; also
