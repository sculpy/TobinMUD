...
- [ ] Implement missing skills from docs/Spell Assignments.xlsx; take
      ...
      STATUS.md Session 141). Batch C still open, real subsystem/
      design gaps disclosed in Session 141: sharpen/smooth (needs a
      weapon sharpness stat), alcoholism (needs a drunk/intoxication
      system), the Deikhan mounted-combat trio, Ranger beast-charm
      pair (blocked on the Pet/charm system, task 35), and 5 Shaman/
      Druid spells (Flatulence/Living Vines/Raze/Shield Of Mists/
      Thornflesh).
...
Ran a scripted cross-check of all 300 unique skill/spell roster names (skill.c) against literal-string usage across src/cmd/*.c + combat.c. 80 had no direct match, but manual review shows most are spells (acid blast, blizzard, chain lightning, heal/harm tiers, protection from *, etc.) -- cmd_cast.c's cast path (line ~2459) calls skill_learn_from_doing() generically off the DB-driven spell lookup, not per literal name, so those are false positives, already covered.

Real candidates for task 14 (stubbed skills) -- skill-shaped roster names with no generic dispatch to fall back on, meaning if they have no cmd_*.c handler they likely do nothing at all: pick lock, shoulder throw, counter steal, retreat, close quarters fighting, groundfighting, power move, sling shot, stunning arrow, two-handed specialization, set trap (arrow/container/grenade/mine), knot, dufali, voplat, snofalte, Garmul's tail, Oomlat Philosophy. Not yet verified live one-by-one -- next session should check being_knows_skill()/skill_find() call sites (or their total absence) for each before assuming it's a real gap vs. a differently-named existing command.
...
- [x] **Pet / charm** — done 2026-07-26 (home). Scoped via
      ...
      warnings.
- [x] **Planting** — done 2026-07-26. Scoped via AskUserQuestion: both
      mechanics in one pass, full 15-plant-type depth. Seed farming
      (`plant <seeds>`, outdoors, new `obj_plant.h`/`.c` + `planting.h`/
      `.c`) ports TPlant as an ephemeral obj_t reusing the generic
      `val[4]` payload, all 15 real seed/fruit vnums, real-year
      lifespans compressed to ~1-real-minute growth ticks (disclosed,
      ordering preserved). Thief `plant <item> <victim>` (reverse
      pickpocket) gated on the "plant" skill itself, PC-vs-PC targeting
      reusing combat.c's mutual `toggle pk` gate (now exported as
      `combat_pk_allowed()`). Two real bugs caught live: a full
      `MAX_PULSE_PROCESSES` table silently dropping one of the two new
      tick registrations (bumped 24->32), and the sowing task's own
      "still have the seeds?" safety check misfiring right after its own
      legitimate consumption of them. New `tests/smoke_test_plant.py`
      (23 checks). See STATUS.md Session 68 for the full writeup,
