...
('settrap', 'Usage: settrap <direction|container|arrow|mine|grenade>\n\nA Thief rigs a trap on a closed door in that direction ("set trap\n(door)"), a closed container ("set trap (container)"), a carried\narrow ("set trap (arrow)"), a carried grenade item ("set trap\n(grenade)"), or the CURRENT ROOM''s own floor ("set trap (mine)", no\nitem or direction needed). A door/container trap springs on\nwhoever walks through/opens it; an arrow trap springs on whoever it\nis shot at and hits; a grenade trap springs on whoever it is thrown\nat and hits (see `throw`); a mine springs on whoever next enters the\nroom from ANY direction. Each is a one-shot hit to a random limb,\nand a Thief who knows "detect trap" safely steps around a rigged\ndoor, container, or mine instead (an arrow or grenade already in\nflight cannot be detected).\n\nRelated: disarmtrap, throw', 'seed'),
('disarmtrap', 'Usage: disarmtrap <direction|container|arrow|mine|grenade>\n\nA Thief who knows "disarm trap" safely removes a trap rigged on a\ndoor, a container, a carried arrow or grenade, or the current room''s\nown floor (mine), with no risk of springing it.\n\nRelated: settrap', 'seed'),
('throw', 'Usage: throw <item> <target>\n\nHurls a loose carried throwable item (a real WEAR_THROW-flagged\nweapon -- a dagger, knife, dart, ... -- or any grenade item) at a\ntarget in the room, spending it as one-shot ammo the same way `shoot`\nspends an arrow. Opens a fight if you weren''t already in one, like\n`shoot`/`kick`. A grenade rigged with `settrap grenade` springs its\ntrap on a landed hit.\n\nRelated: settrap, disarmtrap', 'seed'),
...
UPDATE `help_topic` SET `body` = 'Usage: settrap <direction|container|arrow|mine|grenade>\n\nA Thief attempts to rig a trap on a closed door in that direction\n("set trap (door)"), a closed container ("set trap (container)"), a\ncarried arrow ("set trap (arrow)"), a carried grenade item ("set\ntrap (grenade)"), or the CURRENT ROOM''s own floor ("set trap\n(mine)", no item or direction needed) -- each needs its own skill.\nSuccess depends on your proficiency with that skill (see `skills`) --\na fumbled attempt wastes the try but climbs your proficiency toward\nsuccess next time regardless. Once rigged, a door/container trap\nsprings on whoever walks through/opens it next, an arrow trap\nsprings on whoever it is shot at and hits, a grenade trap springs on\nwhoever it is thrown at and hits (see `throw`), and a mine springs\non whoever next enters the room from ANY direction -- a one-shot hit\nto a random limb each time. A Thief who knows "detect trap" safely\nsteps around a rigged door, container, or mine instead, leaving it\nrigged for the next person (an arrow or grenade already in flight\ncannot be detected).\n\nRelated: disarmtrap, throw'
  WHERE `name` = 'settrap' AND `updated_by` = 'seed';

UPDATE `help_topic` SET `body` = 'Usage: disarmtrap <direction|container|arrow|mine|grenade>\n\nA Thief who knows "disarm trap" attempts to safely remove a trap\nrigged on a door, a container, a carried arrow or grenade, or the\ncurrent room''s own floor (mine), with no risk of springing it\neither way. Success depends on your proficiency with the skill (see\n`skills`) -- a fumbled attempt leaves the trap still rigged, but\nclimbs your proficiency toward success next time regardless.\n\nRelated: settrap'
...
ON DUPLICATE KEY UPDATE `name` = `name`;
-- Generic combat passives + fast heal (missing-skill audit,
-- generic/cross-class, 2026-08-10). See skill.c/combat.c/regen.c.
INSERT INTO `help_topic` (`name`, `body`, `updated_by`) VALUES
('offense', 'A passive combat skill, known from level 1 by every class. Improves\nwith use (learn-by-doing) every time you land a swing, making your\nattacks a little more accurate over time. No command to use it; it\njust works in a fight. Related: advanced offense tactics defense', 'seed'),
('advanced offense', 'A passive combat skill, known by every class in the advanced tier.\nThe stronger sibling of `offense`: a larger to-hit bonus that grows\nas you fight. No command to use it; it trains and applies on its\nown during combat. Related: offense inevitability', 'seed'),
('advanced defense', 'A passive combat skill, known by every class in the advanced tier.\nMakes you meaningfully harder to hit, on top of `defense` and\n`focused avoidance`. Improves with use (learn-by-doing) as blows\nare aimed at you. No command to use it; it just works. Related: defense focused avoidance toughness', 'seed'),
('tactics', 'A passive combat skill, known from level 1 by every class. A general\nsense for a fight that lends a small edge to your accuracy, growing\nslowly with use. No command to use it; it trains on its own while\nyou fight. Related: offense', 'seed'),
...
-- Migration: practice spend now costs two points per 1-2% step (halved
-- pace); restores the full body incl. the `practice <class>` section.
UPDATE `help_topic` SET `body` = 'Usage: practice                          (status)\n       practice <discipline>             (skill listing + proficiency)\n       practice <discipline> <count>     (spend points, at a guildmaster)\n       practice <class> [<discipline>]   (see ANY class''s skill listing)\n\nThree disciplines exist -- Basic, Combat, Advanced -- each raised by\nspending practice points at its own tier of guildmaster: Basic at\nlevel-51 guildmasters (`goto guildmaster`), Combat at level-80\n(`goto combat`), Advanced at level-100 (find them yourself). You earn\npractice points on level-up (scaled by your Wisdom attribute). Your\nown class name works as a synonym for Basic (matching how `skills`\nlabels that tier, e.g. "Warrior Skills"). Advanced is locked until\nBOTH Basic and Combat reach 100%.\n\n`practice <discipline>` -- e.g. `practice combat` -- shows that\ndiscipline''s skill/spell listing with each accessible skill''s own\nindividual PROFICIENCY percentage (see `help skills`), ANYWHERE, no\nguildmaster required. Standing in front of the matching guildmaster\nadds a reminder to spend points there.\n\n`practice <discipline> <count>` -- e.g. `practice combat 5` -- is the\nonly form that actually SPENDS points, raising the discipline\npercentage itself (which acts as the ceiling every skill''s own\nproficiency climbs toward through use). Every one-to-two-percent step now costs TWO points, so mastering a\ndiscipline takes about twice as long as it once did; a lone leftover\npoint carries over to your next visit. The spend stops early if you\nrun low on points or hit 100%. This form needs the matching guildmaster\npresent.\n\n`practice <class>` -- e.g. `practice warrior` -- browses ANY class''s\nBasic listing, not just your own (defaults to Basic; add a tier word,\ne.g. `practice warrior combat`, for Combat/Advanced). Someone else''s\nclass shows as a plain reference (skill names + minimum level, no\nproficiency -- you don''t have access) and can never be spent on. An\nimmortal granted every class at once sees the full listing for any\nclass they ask for instead, since they genuinely have access to all\nof them.\n\nSyntax: practice              (status: all three percentages + points)\n        practice basic         (Basic''s skill listing + proficiency)\n        practice combat 8      (spend up to 8 points on Combat)\n        practice mage           (see the Mage skill listing, read-only)\n\nMinimum Level: 1\n\nRelated: skills cast pray goto'
  WHERE `name` = 'practice' AND `updated_by` = 'seed';
