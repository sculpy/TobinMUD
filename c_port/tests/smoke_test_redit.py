...
# --- Part 1: the gate ---
send_line(s, "edit")
check("Huh?!" in recv_all(s), "a mortal typing edit gets Huh?! (hidden)")
...
recv_all(s)
send_line(s, "edit")
check("Huh?!" in recv_all(s), "a level-51 immortal still can't edit (gate is 56)")
...
check("Room Name: Sandbox Origin" in out and f"Number: {BASE}" in out,
      "bare edit shows the room summary (Sneezy's info block)")
check("NONE" in out, "the summary shows no exits yet")

# --- Part 3: name, sector, description ---
send_line(s, f"edit name The Builder's Workshop")
out = recv_all(s)
check("New Room Title: The Builder's Workshop" in out,
      "edit name confirms with the original's 'New Room Title' message")

send_line(s, "edit sector_type 3")
out = recv_all(s)
check("Sector type set" in out, "edit sector_type sets the sector")
send_line(s, "edit sector_type")
out = recv_all(s)
check("Current sector type: 3" in out, "edit sector_type with no arg shows the value")

send_line(s, "edit description")
out = recv_all(s)
check("Editing room" in out, "edit description opens the line editor")
...
send_line(s, f"edit exit north {BASE + 1}")
out = recv_all(s)
...
send_line(s, f"edit exit north -1")
out = recv_all(s)
check("Deleting exit." in out, "edit exit <dir> -1 deletes the exit")
send_line(s, "north")
out = recv_all(s)
