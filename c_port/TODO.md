...
- [ ] **Custom MUD client + web play client + generated web help/manuals**
      ...
      **Hosting decided 2026-08-10 (user):** reuse the existing droplet
      and domain for both -- tobinmud.com already resolves here and nginx
      already serves `~/TobinMUD/web/` over certbot-managed TLS, so
      neither piece needs a new subdomain, certificate, or firewall
      opening (ufw AND the DigitalOcean cloud firewall both already allow
      80/443 -- see ENVIRONMENT.md's note that this box runs ufw despite
      being Fedora). Web play client (#2): add an nginx `location` block
      reverse-proxying a WebSocket upgrade to a small websocket-to-telnet
      bridge process bound to 127.0.0.1 only -- never exposed directly --
      which in turn talks to 127.0.0.1:4000. The bridge itself is still
      unbuilt; this decision just unblocks it. Web help/manuals (#3):
      generate static HTML into a subdirectory of the existing
      `~/TobinMUD/web/` root, reusing the same generation approach
      `web/generate_help_json.sh` already uses against the `help_topic`
      table -- already live-served, so no hosting work at all.
