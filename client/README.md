...
# 4. Publish -- the nginx web root is /home/mud/TobinMUD/web (repointed
#    2026-08-xx for the help/news site; the old /usr/share/nginx/html
#    path is NO LONGER served -- publishing there silently 404s, which
#    once broke both auto-update and the website download link).
mkdir -p /home/mud/TobinMUD/web/tobinclient
cp TobinMUDClient.msi /home/mud/TobinMUD/web/tobinclient/TobinMUDClient.msi
printf "X.Y.Z" > /home/mud/TobinMUD/web/tobinclient/version.txt
chmod -R a+rX /home/mud/TobinMUD/web/tobinclient
```
The site forces https (301), but the client fetches the http URL and
WinINet will not reliably follow http->https. `/etc/nginx/conf.d/
tobinmud.conf`'s port-80 block therefore serves `location /tobinclient/`
directly (no redirect) while redirecting everything else to https --
keep that exception if the cert config is ever regenerated by Certbot.
...
- `nginx` installed (`sudo dnf install -y nginx`), serving its default
  ...
  --now nginx`).
- Files published to `/home/mud/TobinMUD/web/tobinclient/` (the live
  web root; the old `/usr/share/nginx/html/tobinclient/` is orphaned).
