...
- [ ] **New-player HP formula needs balancing** -- user, 2026-08-06: reported a level-3 character with 28 max HP feels too low. Current formula (being.c's being_calc_max_hp()): `20 + CON bonus + level * class_hp_per_level(class) * balance scale` -- Mage's rate is the lowest of all classes at 5.0/level, so a level-3 Mage lands around there by design; 28 specifically implies a below-average CON roll on top of that. User was asked whether to raise the flat base, raise per-level rates, or something else, and dismissed the question without picking -- needs their actual numbers/preference before changing anything.
