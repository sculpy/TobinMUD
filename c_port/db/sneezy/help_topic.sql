...
('practice', 'Usage: practice\n\nReaching a spell or skill''s level requirement is not enough on its\nown -- you also need to have practiced that discipline. Standing at a\nguildmaster mob of your own class (recognized by the "guildmaster"\nkeyword) and typing `practice` raises your Basic and Advanced\ndiscipline percentages by 10% per visit. A Basic-tier skill or spell\nneeds any Basic percentage above 0; an Advanced-tier one needs Basic\nat 95%+ AND some Advanced percentage. `skills` shows both percentages\nand marks exactly what is still locked and why. Immortals bypass this\ngate entirely, same as the level gate.\n\nRelated: skills cast pray', 'seed'),
...
INSERT INTO `help_topic` (`name`, `body`, `updated_by`) VALUES
('read', 'Usage: read   |   read <#>   |   read at <board name> [<#>]\n\nBulletin boards are mounted in rooms across the world. Standing next\nto one, bare `read` lists every message posted on it; `read <#>`\nshows one in full. Some boards need real rank to use at all -- try\nanyway and it''ll say so if you don''t qualify.\n\nIf a room happens to have more than one board, name which one you\nmean: `read at wiz` (matches by any part of the board''s own name),\nthen optionally a message number, e.g. `read at wiz 3`. With only one\nboard in the room this is never needed.\n\nRelated: write affects', 'seed'),
('write', 'Usage: write <subject> <message>   |   write at <board name> <subject> <message>\n\nPosts a new message on the bulletin board in your current room --\nthe first word is the subject, everything after is the body, all on\none line. Signed with your own name automatically. "board" itself\nisn''t allowed as a subject.\n\nIf the room has more than one board, name which one first: `write at\nwiz <subject> <message>`. With only one board present this is never\nneeded, even if your subject happens to start with the word "at".\n\nRelated: read', 'seed')
ON DUPLICATE KEY UPDATE `name` = `name`;
