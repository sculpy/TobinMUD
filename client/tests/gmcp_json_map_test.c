 ...
 * ignored rather than corrupting an array slot, and the whole table
 * survives a save/load round trip byte-for-byte in content. */
...
int main(void) {
    ...
    check(map_model_upsert(&m, 942900, "A bare sandbox room.", exits),
    ...
    exits2[0] = -1;
    map_model_upsert(&m, 942900, "A bare sandbox room.", exits2);
    ...
    map_model_upsert(&m, 942901, "A dusty hallway.", exits2);
