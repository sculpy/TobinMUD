...
- [x] Fast load -- DONE 2026-08-17: built ranged combat (cmd_shoot) and attached Fast load to it; Tobin now HAS a ranged-combat system (same reason Beast summon deferred). Revisit if/when ranged combat is built.
...
- Lifeforce in the client status bar reports mana, change to lifeforce for druids -- DONE 2026-08-17: built a real scaling Lifeforce pool (lifeforce skill drives being_calc_max_mana; cmd_cast trains it) and relabeled the client gauge dynamically off a new GMCP/MSDP resource label (client rebuilt + MSI republished, version 0.4.31). ORIGINAL NOTE: Tobin has no real Lifeforce resource pool (druid max_mana is always 0; score/cast only relabel the word -- see resource_pool_label()/cmd_cast.c:3061), and the status bar lives in the Windows client (needs an MSI rebuild). Proper fix = build a Lifeforce resource, then relabel client-side. Revisit with the mana/casting items (#689/#691).
...
- Make room flags and sector types have their intended effects from Sneezy (user 2026-08-16). [ STARTED 2026-08-16: ROOM_FLAG_DEATH (deathtrap kill on entry, combat_death_room_kill_pc) and ROOM_FLAG_PRIVATE (2-player entry cap) now act in cmd_move.c; see smoke_test_room_flags.py.
  ...
  (c) DONE 2026-08-17 -- per-sector thirst/hunger drain: sector_thirst_rate/sector_hunger_rate (room.c), ported in spirit from the original TerrainInfo thirst/hunger columns fed to foodNDrink() -- deserts/savannah parch, mountains/climbing/forest starve; baseline 2 leaves ordinary terrain unchanged. Wired into vitals.c drain tick; surfaced in the immortal look header. Guarded by smoke_test_sector_effects.py (4 checks).
  (c-heat) DONE 2026-08-17 -- Tobin-ORIGINAL heat subsystem (NOT a port: upstream defines TerrainInfo heat but no engine code reads it; user opted to build it as a labelled Tobin invention). sector_heat() (room.c) buckets ambient heat on the original data's scale (lava 140 .. arctic -30). vitals.c chips 1 HP/tick past HEAT_DAMAGE_HOT/COLD outdoors (heatstroke/hypothermia), saved by a race RESIST_HEAT/COLD roll, sheltered by ROOM_FLAG_INDOORS; cmd_move.c adds a cosmetic sweat/shiver cue past the milder STRESS band. Immortal look header shows heatN. Guarded by smoke_test_heat.py (8 checks). DEFERRED within this: gear insulation, weather coupling.
  NOTE: the earlier "no-mob/peaceful sectors" idea was dropped -- ROOM_FLAG_NO_MOB already exists AND is enforced in mob wandering (mob_ai.c:165), and there is no upstream PEACEFUL-sector concept to port (peaceful is a room flag upstream, not a sector). ]
